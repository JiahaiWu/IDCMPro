...
namespace IDCM.DynamicDB.ComPO
{
    /// <summary>
    ...
    /// </summary>
    [Serializable]
    public class TColumnMapping : ICloneable
    {
        ...
        /// </summary>
        /// <returns></returns>
        public TColMappingEnum GetEnumerator()
        {
            return new TColMappingEnum(colMapping.OrderBy(rs => rs.Value.Val).ToArray());
        }
        ...
        /// 用于支持迭代器迭代的辅助类实现
        /// </summary>
        public class TColMappingEnum : IEnumerator
        {
            private KeyValuePair<string, ObjectPair<int, int>>[] __colMapping = null;
            private int position = -1;
            public TColMappingEnum(KeyValuePair<string, ObjectPair<int, int>>[] _colMapping)
            {
