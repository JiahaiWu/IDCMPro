...
namespace DCMControlLib
{
    ...
    public class DCMComboBox : ComboBox, IPopupControlHost
    {
            ...
            : base()
        {
            ...
            this.DrawItem += new DrawItemEventHandler(OnComboBoxDrawItem);
        ...
        public DCMComboBox(Control dropControl)
            : this()
        {
        ...
        protected void OnComboBoxDrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            int wh = e.Bounds.Height;
            // Let's highlight the currently selected item like any well 
            // behaved combo box should
            Pen pen=new Pen(Color.GhostWhite,3);
            switch (m_val_status)
            {
                case ValidationStatus.Ok:
                    e.Graphics.FillRectangle(Brushes.LightSeaGreen, e.Bounds);
                    e.Graphics.DrawString(this.Text, this.Font, Brushes.Black,new Point(wh + 8, e.Bounds.Y));
                    break;
                case ValidationStatus.Error:
                    e.Graphics.FillRectangle(Brushes.Red, e.Bounds);
                    e.Graphics.DrawString(this.Text, this.Font, Brushes.Black,new Point(wh + 8, e.Bounds.Y));
                    e.Graphics.DrawLine(pen, wh + 8, e.Bounds.Y + wh / 2, this.Width - this.DefaultPadding.Right, e.Bounds.Y + wh / 2);
                    break;
                case ValidationStatus.Warn:
                    e.Graphics.FillRectangle(Brushes.YellowGreen, e.Bounds);
                    e.Graphics.DrawString(this.Text, this.Font, Brushes.Black,new Point(wh + 8, e.Bounds.Y));
                    break;
                case ValidationStatus.UnChecked:
                default:
        ...
        [Category("Custom Validation"), Description("Indicates current value validation result."), DefaultValue(ValidationStatus.UnChecked)]
        public ValidationStatus ValueStatus
        {
            get { return this.m_val_status; }
            set
            {
                if (value != this.m_val_status)
                {
                    this.m_val_status = value;
                    this.Invalidate();//AutoRenderStatus();
                }
            }
        }
        ...
        /// <summary>
        /// Indicates current value status.
        /// </summary>
        ValidationStatus m_val_status = ValidationStatus.UnChecked;
        /// <summary>
